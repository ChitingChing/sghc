...
public class RegistroInformacionController implements Initializable {
    ...
    public void llenarCombos(){
        ...
        chkcmbPatologiasParto.getItems().setAll(uDao.getPatologias("parto"));
        //trabajo de parto
        //Recien Nacido
    ...
    public void guardar() {
       ...
       // v.getValidationResult().getErrors().stream().
        try {
            ...
            m2.showError("Ha ocurrido un error.",ex);
            Stage s = (Stage) txtTelefono.getScene().getWindow();
            s.close();
        }
        em.close();
    ...
    private void llenarInfo(Paciente p){
        if(!editar) {
            ...
            Integer nhistorialclinico =  em.createNativeQuery("Select coalesce(max(nhistorialclinico),0)+1 From Paciente").getFirstResult();
            p.setnHistorialClinico(nhistorialclinico);
        }
    ...
    private void llenarAntecedentes(Antecedente a, Paciente p){
        ...
        a.setPaciente(p);
        List<Antecedente> listAnt = new ArrayList<>();
        listAnt.add(a);
        p.setAntecedenteCollection(listAnt);
    }
    public void llenarEmbarazo(Embarazo e,Paciente p) {
        ...
        e.setPaciente(p);
        List<Embarazo> listEmb = new ArrayList<>();
        listEmb.add(e);
        p.setEmbarazoCollection(listEmb);
    }
    public void llenarParto(Embarazo e){
        ...
        Collection<AdmisionPorParto> listaParto = new ArrayList<>();
    ...
    private void llenarRecienNacido(){
        ...
        Collection<RecienNacido> listaRecienNacido = new ArrayList<>();
    ...
    private void llenarEgreso(){
        ...
        Collection<EgresoRecienNacido> listaEgresoRecienNacido = new ArrayList<>();
    ...
    public void borrarPuerperio() {
        if(gridPuerperio.getSelectionModel().isEmpty()){
            Message m = new Message();
            m.showMessage("Info","Seleccione una fila primero");
        }else{
    ...
    public void init(UUID IdPaciente) {
        try {
            //cargo datos a editar pacienta y antecedentes
            em = Connection.emf.createEntityManager();
            ...
            txtTelefono.setText(p.getTelefono1());
            if (!p.getProvincia().isEmpty())
                cmbProvincia.getSelectionModel().select(p.getProvincia());
            if (!p.getCanton().isEmpty())
                cmbCanton.getSelectionModel().select(p.getCanton());
            if (!p.getEstudios().isEmpty())
                cmbEstudios.getSelectionModel().select(p.getEstudios());
            if(!p.getEstadoCivil().isEmpty())
                cmbEstadoCivil.getSelectionModel().select(p.getEstadoCivil());
            try {
                a = (Antecedente) em.createQuery("From Antecedente a where a.pacienteid='" + p.getId() + "'").getSingleResult();
            ...
            //chkcmbAntecedentesFamiliares.getCheckModel().check();
            em.close();
            editar = true;
        }catch(Exception ex){
            Message m = new Message();
