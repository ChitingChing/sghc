            ...
            row.setOnMouseClicked(event -> {
                if(!row.isEmpty() && event.getClickCount()==2) {
                 //  tblVisitas.setItems(getVisitas(row.getItem().getId().toString()));
                    ...
                    m.showMessage("hey",String.valueOf(row.getTableView().getSelectionModel().getSelectedItem().getEmbarazoCollection().stream().count()));
                }
            });
            return row;
        });
...
//                    "like '%"+txtTextoBuscar.getText().trim()+"% or p. ";

            ObservableList<Paciente> listaPa=null;
//            EntityManager em = Connection.emf.createEntityManager();
            PacienteDao pDao = new PacienteDao();
            listaPa =FXCollections.observableArrayList(pDao.getListaPacientes(txtTextoBuscar.getText()));
            tblPacientes.setItems(listaPa);

            //em.close();
        } catch (Exception ex) {
            Message m = new Message();
            m.showError("Ha ocurrido un error.",ex);
//            Stage s = (Stage) txtNombres.getScene().getWindow();
//            s.close();
        }
...
//            s.close();
    ...
    public void showNuevoEmbarazo() throws IOException {
        if(!tblPacientes.getSelectionModel().isEmpty()){
            ...
            Stage stageSearch = (Stage) txtTextoBuscar.getScene().getWindow();
            stageSearch.close();
